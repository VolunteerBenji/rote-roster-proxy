...
async function signedComlinkRequest(path, bodyObj) {
  ...
  // there usually means "not ready yet", not a real failure. Retry several times
  // with increasing waits before giving up. Real-world testing showed a cold
  // start can take close to a minute, so this budget runs up to ~100 seconds
  // total rather than cutting off too early.
  const RETRY_DELAYS_MS = [5000, 10000, 15000, 25000, 30000]; // up to 5 retries after the first try
  let resp, lastErr;
